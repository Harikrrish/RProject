...
export async function renderGallery({ root = process.cwd() } = {}) {
  ...
  const cards = gallery.items.map((item, i) => `<a class="design-card" data-gallery-id="${escape(item.id)}" data-kind="${item.kind === 'generated-concept' ? 'generated-concept' : 'collection'}" data-category="${categoryKey(item.category)}" data-label="${escape(item.label)}" data-room="${escape(item.category)}" href="${escape(item.full)}" aria-label="View ${escape(item.alt)}"><div class="design-image"><img src="${escape(item.thumbnail)}" alt="${escape(item.alt)}" width="${item.thumbnailWidth}" height="${item.thumbnailHeight}" loading="lazy" decoding="async"><span class="image-arrow" aria-hidden="true">⤢</span><span class="design-room">${escape(item.category)}</span></div><div class="design-card-caption"><div><h3>${escape(item.label)}</h3></div><span>${String(i + 1).padStart(2,'0')}</span></div></a>`).join('\n');
  const section = `<section class="work-section section-space" id="ourdesign"><div class="container">
<div class="section-heading"><div><p class="eyebrow"><span class="orange-dash"></span> Designs to explore</p><h2>More spaces.<br><em>More possibilities.</em></h2></div><p>Explore our complete collection of living rooms, kitchens, bedrooms and more. Find the details you would love to bring home.</p></div>
